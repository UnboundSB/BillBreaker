    ...
    <div className="space-y-8 pb-20">
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm opacity-90 mt-1">
              Due to sub-penny rounding on shared items, the total assigned amount differs from the printed receipt by ${Math.abs(mismatch).toFixed(2)}. 
          ...
          >
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between text-text-secondary">
                ...
                <span className="font-medium text-text-primary">${Number(person.items_total).toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-text-secondary">
                <span>Tax</span>
                <span className="font-medium text-text-primary">${Number(person.tax).toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-text-secondary">
                <span>Service / Tip</span>
                <span className="font-medium text-text-primary">${Number(person.service_charge).toFixed(2)}</span>
              </div>
              {Number(person.discount) > 0 && (
                <div className="flex justify-between text-emerald-400">
                  <span>Discount</span>
                  <span className="font-medium">-${Number(person.discount).toFixed(2)}</span>
                </div>
              )}
            </div>
          </motion.div>
      ...
      <div className="glass-panel p-6 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm text-text-secondary">Printed Total: <span className="text-text-primary">${Number(results.printed_total).toFixed(2)}</span></p>
            <p className="text-xs text-text-secondary">Calculated Total: ${Number(results.calculated_total).toFixed(2)}</p>
          </div>
        </div>
